...
public class MyDefaultObserverEventHandler : MonoBehaviour
{
    ...
    ResetTheModel resetModel;
    PlayTheParticle playParticle;
    Animator scanPicAni;
    ClickTheModelAction clickTheModelAction;


    private void Awake()
    {
        resetModel = transform.Find("Model").GetComponent<ResetTheModel>();
        playParticle = this.GetComponent<PlayTheParticle>();
        scanPicAni = transform.Find("Canvas(世界空间)/扫描").GetComponent<Animator>();
        clickTheModelAction = this.GetComponent<ClickTheModelAction>();

        print("MyDefaultObserverEventHandler:" + this.gameObject.name);
    ...
    protected virtual void Start()
    {
        ...
        if (mObserverBehaviour)
        {
            ...
            OnTargetFound.AddListener(()=>
            {
                ...
                scanPicAni.gameObject.SetActive(true);
                scanPicAni.Play(0);
                playParticle.Play();
            ...
            {
                scanPicAni.gameObject.SetActive(false);
                playParticle.DisappearTheModel();
                clickTheModelAction.HideUI();

                BtnJieShao.Instance.ShowOrHideBtnJieShao(false);
